...
def do_apcluster(sim,dism,name,minc=2,maxc=10,recal=True,auto=True,kin=None,m1=0.5,m2=2.5):
    if recal:
        krange = numpy.linspace(numpy.min(sim)*m1,numpy.min(sim)*m2,30)
        ch = []
        kr = []
        sc = []
        clus=[]
        for k in krange:
            af = AffinityPropagation(preference=k,affinity='precomputed').fit(sim)  # preference=-50,preference=numpy.min(sim),
