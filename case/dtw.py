...
@jit(nopython=True)
def spherical_distance(point1, point2, R=6378, method='sp'):
    # point should be input as [ lat, lon ] ,
    # positive for North , negative for South
    if method == 'sp':
        Pi = 3.1415926
        dlat = np.absolute(point1[0] - point2[0]) / 180 * Pi
        dlon = np.absolute(point1[1] - point2[1]) / 180 * Pi
        lat1 = point1[0] / 180 * Pi
        lat2 = point2[0] / 180 * Pi
        l_p1_p12 = 2 * np.sin(dlon / 2) * np.cos(lat1)
        l_p1_p21 = 2 * np.sin(dlat / 2)
        l_p2_p21 = 2 * np.sin(dlon / 2) * np.cos(lat2)
        l_p1_p2 = np.sqrt(l_p1_p21 ** 2 + l_p2_p21 * l_p1_p12)
        theta = 2 * np.arcsin(1 / 2 * l_p1_p2)
        dis = theta * R
    elif method == 'ed':
        dlat = np.absolute(point1[0] - point2[0])
        dlon = np.absolute(point1[1] - point2[1])
        dis = dlat**2+dlon**2
        dis=dis**0.5
    return dis
...
@jit(nopython=True)
def dtw_distance(series1, series2, method='sp'):
    """
    ...
    # Fill First Cell
    E[0,0] = spherical_distance(series1[0] , series2[0],method=method)

    # Fill First Column
    for i in range(1, l1):
        E[i,0] = E[i - 1,0] + spherical_distance(series1[i] , series2[0],method=method)

    # Fill First Row
    for i in range(1, l2):
        E[0,i] = E[0,i - 1] + spherical_distance(series1[0] , series2[i],method=method)

    for i in range(1, l1):
        for j in range(1, l2):
            v = spherical_distance(series1[i] , series2[j],method=method)
